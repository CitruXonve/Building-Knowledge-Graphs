...
for item1 in item_list1:
    for item2 in item_list2:
        if (item1["id"], item2["id"]) in range:
            sim_obs[(item1["id"], item2["id"])] = (title_similarity(get_cached_title(item1["id"], item1["title"]), get_cached_title(item2["id"], item2["title"])), year_similarity(item1["year"], item2["year"]))
            # del range[(item1["id"], item2["id"])]

with open("covered.txt", "w") as fout:
    for item_pair, values in sorted(sim_obs.items()):
        if values[0] >= 0.82 and values[1] >= 1:
            fout.write(f'{item_pair[0]}\t{item_pair[1]}\t{1}\n')
